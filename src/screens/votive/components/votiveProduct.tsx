...
import { Row, Col, Rate, Button, message, Modal } from "antd";
...
export const VotiveProduct: React.FC = () => {
  ...
  useEffect(() => {
    const getProduct = async () => {
      if (votiveId) {
        const resProduct = await productService.findById(votiveId);
        if (resProduct.data) {
  ...
  useEffect(() => {
    ...
    const getComment = async () => {
      ...
      if (res.data) {
        setDataList(
          res.data.filter(
            (comment: ICardDataReview) => comment.productId === votiveId
          )
        );
      }
    };
